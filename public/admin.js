...
function displayUsers(users) {
    ...
    users.forEach(user => {
        ...
        row.innerHTML = `
            ...
            <td>${user.email}</td>
            <td>
                <select class="role-select" data-user-id="${user._id}" onchange="updateUserRole('${user._id}', this.value)">
            ...
            <td>
                <button onclick="deleteUser('${user._id}')" class="delete-btn">删除</button>
            </td>
        `;
...
async function updateUserRole(userId, role) {
    ...
    if (userId === currentUser.id && !confirm('您正在修改自己的角色，确定要继续吗？')) {
        // 重置选择框到之前的状态
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 绑定房间标签点击事件
    document.getElementById('roomsTab').addEventListener('click', fetchRooms);
...
async function loadStats() {
    try {
        ...
        if (roomsResponse.ok) {
            ...
            document.getElementById('totalGames').textContent = rooms.length;
